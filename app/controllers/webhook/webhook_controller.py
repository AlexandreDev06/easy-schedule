...
async def answer_message(session_token: str, data: dict) -> bool:
    ...
    if data["event"] == "onmessage" and data["isGroupMsg"] == False:
        print(session_token)
        current_user = await BaseCrud(User).get_record(filters=[User.session_token == session_token], unique=True)
        if current_user.is_active:
            print(data["from"].split("@")[0], data["content"])
            await MessagesWppServices(current_user).answer(data["from"].split("@")[0], data["content"])
